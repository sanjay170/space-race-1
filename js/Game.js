class Game {
  ...
  start() {
    ...
    ship1 = createSprite(width / 2 - 50, height - 100);
    ship1.addImage("ship1", ship1img);
    ship1.scale = 0.07;
    ship1.addImage("blast", blastImage); 

    ship2 = createSprite(width / 2 + 100, height - 100);
    ship2.addImage("ship2", ship2img);
    ship2.scale = 0.07;
    ship2.addImage("blast", blastImage); 
  ...
  play() {
    ...
    if (allPlayers !== undefined) {
      ...
      for (var plr in allPlayers) {
        ...
        if (currentlife <= 0) {
          ships[index - 1].changeImage("blast");
        ...
        if (index === player.index) {
          ...
          camera.position.y = ships[index - 1].position.y;
        }
      }
  ...
  handleFuel(index) {
    ...
    ships[index - 1].overlap(fuels, function(collector, collected) {
      player.fuels = 185;
    ...
    if (player.fuels > 0 && this.playerMoving) {
      player.fuels -= 0.3;
    }
  ...
  handlePowerCoins(index) {
    ships[index - 1].overlap(powerCoins, function(collector, collected) {
      player.score += 21;
      player.update();
  ...
  showFuelBar() {
    ...
    fill("#ffc400");
    rect(width / 2 - 100, height - player.positionY - 200, player.fuels, 20);
    noStroke();
    pop();
  }
  ...
  handleObstacleCollision(index) {
    if (ships[index - 1].collide(obstacles)) {
